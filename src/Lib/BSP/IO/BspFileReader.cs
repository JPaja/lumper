...
namespace Lumper.Lib.BSP.IO
{
    public class BspFileReader : LumpReader
    {
        ...
        protected override void ReadHeader()
        {
            ...
            //finding the real gamelump length
            Lump gameLump = null;
            ...
            if (CheckOverlapping())
                throw new InvalidDataException("overlapping lumps bad");
        }
        ...
        private void SortLumps()
        {
            ...
            foreach (var l in Lumps.OrderBy(x => x.Item2.Offset))
            {
                var temp = _bsp.Lumps.Where(x => x.Value == l.Item1).First();
                newLumps.Add(temp.Key, temp.Value);
                _bsp.Lumps.Remove(temp.Key);
            }
            if (_bsp.Lumps.Any())
                throw new InvalidDataException("bsp lumps and reader headers didn't match!");
            _bsp.Lumps = newLumps;
